...
public class SigRoutes5_20 {
	 ...
	 * @param graph
	 */
	public SigRoutes5_20(Graph graph, double mcIterations, double pvalue) {		
		this.graph = graph;
	...
	public ArrayList<Path> getSignificantRoutes(boolean likelihoodPruning,
		...
		if (dynamicSegmentation && graph.numActivities > graph.originalNodes.size()) {
			...
			for (int i = 0; i < graph.originalNodes.size(); i++) {		
				String currentNode = graph.originalNodes.get(i);
				sp.processAllSPNodes(currentNode);
				for (int j = 0; j < graph.originalNodes.size(); j++) {		
					if (graph.originalNodes.get(j) != currentNode) {
						ArrayList<Edge> a = sp.getShortestPathEdgesNoProcessing(currentNode, graph.originalNodes.get(j));				
						if (a.size() > 0) {
							Path p = new Path(a);					
							spTable.put("node"+currentNode+",node"+graph.originalNodes.get(j), p); //store shortest path for later					
						}
	...
	public static void main (String[] args) throws CloneNotSupportedException, IOException {		
		...
		SigRoutes5_20 routeScan = new SigRoutes5_20(g,numSimulations,pvalue);		
